...
export class App {
  ...
  // 遍历组件
  static traverse(poster: any, search: any) {
    const config = workspace.getConfiguration('zlst');
    let vueFiles: any[] = [];
    let cond = null;
  ...
  // 遍历处理
  static traverseHandle(postPath: string, vueFiles: any [], prefix: any, poster: string, search: string) {
  ...
  // 遍历添加
  static traverseAdd(rootPath: string, dir: string, vueFiles: any[], prefix: any, poster: string, search: string) {
...
export class ElementCompletionItemProvider implements CompletionItemProvider {
  ...
  provideCompletionItems(document: TextDocument, position: Position, token: CancellationToken): ProviderResult<CompletionItem[] | CompletionList> {
    ...
    const config = workspace.getConfiguration('zlst');
    this.size = <number>config.get('indent-size');
...
export class DocumentHoverProvider implements HoverProvider {
  // 获取属性所属标签
  static getTag(document: any, position: any): String {
