...
export default class PageNew {
  ...
  public static setPage(context: vscode.ExtensionContext) {
    ...
    try {
      configPath = Util.winRootPathHandle(configPath);
    ...
    if (config.rootPathLocalPage) {
      ...
      try {
        localPageConfig = Util.winRootPathHandle(localPageConfig);
      ...
      try {
        ...
        if (has[0]) {
          ...
          try {
            ...
            if (filePath.endsWith('wxml')) {
              ...
              pathXml = Util.winRootPathHandle(pathXml);
        ...
        if (has[1]) {
          ...
          try {
            ...
            if (filePath.endsWith('wxss')) {
              ...
              pathWxss = Util.winRootPathHandle(pathWxss);
        ...
        if (has[2]) {
          ...
          try {
            ...
            pathJson = Util.winRootPathHandle(pathJson);
  ...
  "usingComponents": {
    ${has[2]}
        ...
        try {
          pathJs = Util.winRootPathHandle(pathJs);
...
${space}},\n`;
    ...
    pages.forEach(page => {
      ...
      switch (page.type) {
        ...
        case 'store':
          ...
          try {
            storeJs = Util.winRootPathHandle(storeJs);
            fs.statSync(storeJs);
          } catch (error) {
  ...
  public static async fileGenerate(pagePath: string, template: string, type: string, options: any) {
    // 读取模板文件，替换，生成
    template = Util.winRootPathHandle(template);
  ...
  public static async api(context: vscode.ExtensionContext, uri: vscode.Uri) {
    ...
    if (activeEditor || (sfs && sfs.length === 1)) {
      ...
      apiPathRoot = Util.winRootPathHandle(apiPathRoot);
        ...
        if (apiName) {
          ...
          if (reqMethod) {
            ...
            if (dirs.indexOf(apiPath) !== -1) {
              ...
              apiJs = Util.winRootPathHandle(apiJs);
              try {
