...
export class JsCompletionItemProvider implements CompletionItemProvider {
  public static document: any = null;
  // 获取refs方法
  getRefMethods(documentText: string, tagName: string) {
    // 1. 找出标签所在路径
 ...
 // 获取对象属性和方法
 getFieldMethod(content:any, objName: string) {
  ...
  provideCompletionItems(document: TextDocument, position: Position, token: CancellationToken): ProviderResult<CompletionItem[] | CompletionList> {
    ...
    let config = workspace.getConfiguration('meteor');
    if (config.tips) {
                ...
                if (obj.method.hasOwnProperty(keyMethod)) {
                  if (keyMethod === ret && obj.method[keyMethod].params) {
                    if (Array.isArray(obj.method[keyMethod].params)) {
                      return obj.method[keyMethod].params.map((keyItem: any) => {
                        return {
                          label: keyItem,
                          sortText: `00` + (ret.length),
