...
// 跳转到定义位置
export class MeteorDefinitionProvider implements DefinitionProvider {
  public VUE_ATTR: any = {
    props: 1,
    computed: 2,
    methods: 3,
  ...
  async definitionOutFile(document: TextDocument, file: any) {
    ...
    // 1. 根据文件目录查询是否存在相应文件
    let config: any = workspace.getConfiguration('meteor');
    if (config || config.pathAlias) {
      // 支持的前缀、后缀
      filePath = filePath.replace(config.pathAlias.alias, config.pathAlias.path);
    }
