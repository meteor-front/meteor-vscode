...
class JsCompletionItemProvider {
                ...
                if (propText.length > 0) {
                    propText.forEach((propItem, propIndex) => {
                        ...
                        props.push({
                            ...
                            insertText: new vscode_1.SnippetString(`${protName}($1)$0`),
            ...
            for (const fieldKey in fieldObj.field) {
                if (fieldObj.field.hasOwnProperty(fieldKey)) {
                    ...
                    ret.push({
                        ...
                        insertText: fieldKey,
                        kind: vscode_1.CompletionItemKind.Field
                    });
                }
            }
    ...
    provideCompletionItems(document, position, token) {
        ...
        if (/\.\$refs..+/gi.test(text)) {
            // 获取相应组件, 用于获取组件名
            let docText = document.getText(new vscode_1.Range(new vscode_1.Position(0, 0), new vscode_1.Position(document.lineCount, 0)));
                                        ...
                                        if (Array.isArray(obj.method[keyMethod].params)) {
                                            return obj.method[keyMethod].params.map((keyItem) => {
                                                return {
                                                    label: keyItem,
                                                    sortText: `00` + (ret.length),
                                                    insertText: new vscode_1.SnippetString(keyItem),
                                                    kind: vscode_1.CompletionItemKind.Field
                                                };
                                            });
                                        }
                                            ...
                                            return [{
                                                    ...
                                                    sortText: `00` + (ret.length),
                                                    insertText: new vscode_1.SnippetString(obj.method[keyMethod].params),
                                                    kind: vscode_1.CompletionItemKind.Field
                                                }];
                                        }
                                    }
