...
exports.DocumentHoverProvider = exports.ElementCompletionItemProvider = exports.App = exports.decodeDocsUri = exports.encodeDocsUri = exports.SCHEME = void 0;
...
class App {
    ...
    static traverse(poster, search) {
        const config = vscode_1.workspace.getConfiguration('zlst');
        let vueFiles = [];
        let cond = null;
    ...
    deleteComplete() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (((_d = vscode_1.window.activeTextEditor) === null || _d === void 0 ? void 0 : _d.selection.start.line) === ((_e = vscode_1.window.activeTextEditor) === null || _e === void 0 ? void 0 : _e.selection.end.line)
                && ((_f = vscode_1.window.activeTextEditor) === null || _f === void 0 ? void 0 : _f.selection.start.character) === ((_g = vscode_1.window.activeTextEditor) === null || _g === void 0 ? void 0 : _g.selection.end.character)) {
                // 首行
    ...
    tagFormat() {
        ...
        if (((_a = vscode_1.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.selections.length) === 1) {
            let veturConfig = vscode_1.workspace.getConfiguration('vetur');
            const tabSize = vscode_1.workspace.getConfiguration('editor').tabSize;
            let spaceAdd = '';
            if (veturConfig) {
...
class ElementCompletionItemProvider {
    ...
    getTagSuggestion() {
        ...
        for (let tag in vue_tags_1.TAGS) {
            suggestions.push(this.buildTagSuggestion(tag, vue_tags_1.TAGS[tag], id));
            id++;
        }
        return suggestions;
    }
    ...
    getAttrSuggestion(tag) {
        ...
        for (let attr in vue_attributes_1.ATTRS) {
            const attrItem = this.getAttrItem(tag, attr);
    ...
    buildTagSuggestion(tag, tagVal, id) {
        ...
        function build(tag, { subtags, defaults }, snippets) {
            ...
            // 子标签
            if (subtags) {
                subtags.forEach((item) => build(item, vue_tags_1.TAGS[item], snippets));
                snippets.push(`</${tag}>`);
            }
            else {
                // 关闭标签
    ...
    buildAttrSuggestion({ attr, tag, bind, method }, { description, type, global, framework }) {
        if ((method && type === "method") || (bind && type !== "method") || (!method && !bind)) {
            ...
            if (vue_tags_1.TAGS[tag] && vue_tags_1.TAGS[tag].framework) {
                detail += vue_tags_1.TAGS[tag].framework;
            }
            if (global) {
    ...
    getAttrValues(tag, attr) {
        ...
        if (!options && attrItem) {
            if (attrItem.type === 'boolean') {
                ...
                options = vue_attributes_1.ATTRS['icons'];
            }
            else if (attrItem.type === 'shortcut-icon') {
                options = [];
                vue_attributes_1.ATTRS['icons'].forEach((icon) => {
    ...
    // 获取标签包含的属性
    getTagAttrs(tag) {
        return (vue_tags_1.TAGS[tag] && vue_tags_1.TAGS[tag].attributes) || [];
    }
    // 获取属性项
    getAttrItem(tag, attr) {
        return vue_attributes_1.ATTRS[`${tag}/${attr}`] || (attr && vue_attributes_1.ATTRS[attr]);
    }
    // 属性值开始
    ...
    getCloseTagSuggestion() {
        ...
        if (exclude.indexOf(tag) === -1) {
            (_a = vscode_1.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.edit((editBuilder) => {
                editBuilder.insert(this._position, '</' + tag + '>');
            });
            let newPosition = (_b = vscode_1.window.activeTextEditor) === null || _b === void 0 ? void 0 : _b.selection.active.translate(0, 0);
            if (vscode_1.window.activeTextEditor && newPosition) {
        ...
        if (search) {
            ...
            vueFiles.forEach(vf => {
                ...
                suggestions.push({
                    label: vf.name,
                    sortText: `0${vf.name}`,
                    insertText: new vscode_1.SnippetString(`\${1:${camelName}} from '${filePath}'`),
    ...
    importStateSuggestion() {
        ...
        const config = vscode_1.workspace.getConfiguration('zlst');
        filePath = filePath.replace(config.componentPrefix.alias, config.componentPrefix.path);
        if (!filePath.endsWith('.js')) {
            filePath += '.js';
        }
        try {
            let dist = fs.readFileSync(path.join(vscode_1.workspace.rootPath || '', filePath), 'utf-8');
            ...
            for (let i = 0; i < lineTexts.length; i++) {
                ...
                if (/export.*/gi.test(lienText.trim())) {
                    ...
                    props.push({
                        label: stateName,
                        sortText: '0' + (props.length + 1),
                        insertText: new vscode_1.SnippetString(`${stateName}`),
                        kind: vscode_1.CompletionItemKind.Property,
                        documentation: ''
                    });
                }
...
class DocumentHoverProvider {
    ...
    provideHover(document, position, token) {
        ...
        textMeta = line.text.substr(posIndex, 1);
        // tag标签便利
        if (documents_1.Documents[selectText]) {
            return new vscode_1.Hover(documents_1.Documents[selectText]);
        }
        // 在不是标签元素情况下才获取标签名称
        if (textMeta !== "<") {
            let tagName = DocumentHoverProvider.getTag(document, position);
            if (tagName && documents_attr_1.DocumentsAttr[tagName + '/' + selectText]) {
                return new vscode_1.Hover(documents_attr_1.DocumentsAttr[tagName + '/' + selectText]);
            }
        }
        return null;
    }
